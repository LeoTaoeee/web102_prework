...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++) {
        ...
        gameCard.innerHTML = `
            <img game-img src="${game.img}" alt="${game.name}" />
            <h3>${game.name}</h3>
...
contributionsCard.innerHTML = `
            <h3> ${totalBacker.toLocaleString('en-US')}</h3>
        `;
...
raisedCard.innerHTML = `
            <h3> ${totalRaised.toLocaleString('en-US')}</h3>
        `;
...
gamesCard.innerHTML =`
            <h3> ${totalGames.toLocaleString('en-US')}</h3>
        `;
...
const unfundedMessage = `There ${unfundedCount === 1 ? 'is 1 game' : `are ${unfundedCount} games`} that is not yet funded.`;
